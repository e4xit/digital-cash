...
class Node:
    ...
    def fetch_utxos(self, public_key):
        print('Fetch UTXO using pubkey:', public_key)
        print('Returning UTXOs:', [tx_out for tx_out in self.utxo_set.values()
                                   if tx_out.public_key == public_key])
        print(len(self.utxo_set))
    ...
    def fetch_balance(self, public_key):
        ...
        print('Fetching balance using pubkey:', public_key)

        # Sum the amounts
        print("sum of UTXOs:", sum([tx_out.amount for tx_out in utxos]))
    ...
    def handle_block(self, block):
        ...
        # validate the coinbase separately
...
class TCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        logger.info(f"received {command} from '{peer[0]}'")

        # Handshake / Authentication
        if command == "connect":
            if peer not in node.pending_peers and peer not in node.peers:
                node.pending_peers.append(peer)
                logger.info(f'(handshake) Accepting "connect" request from "{peer[0]}"')
            ...
            assert peer in node.peers, \
                logger.info(f'Rejecting {command} from unconnected "{peer[0]}"')
        ...
        if command == "sync":
            ...
            # But which builds off a block they do know about
            peer_block_ids = data
...
def lookup_private_key(keyword):
    """
    Hacky way to predictably lookup private keys for "node ids"
    ...
    # Otherwise, look up in this little registry of "characters"
        ...
        exponent = {
            "alice": 1, "bob": 2, "node0": 3, "node1": 4, "node2": 5}[keyword]
...
def main(args):
    if args["serve"]:
        ...
        try:
            ...
            logger.info(print("Please set the environment variable NAME"))
            sys.exit(1)
        ...
        # Alice is Satoshi
        alice_pub_key = lookup_public_key("alice")
        ...
        # Join the network
        peers = [(p, PORT) for p in os.environ.get('PEERS', '').split(',')]
        for peer in peers:
            node.connect(peer)
        ...
        # logger.info(print(public_key.to_string()))
        address = external_address(args["--node"])
